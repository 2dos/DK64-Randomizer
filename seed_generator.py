...
def randomize(post_data):
    ...
    Args:
        post_data (dict): Post data from the form.
    """
    levelEntrances = [
        "Jungle Japes",
        "Angry Aztec",
    ...
    with open(f"{sys.path[0]}/static/patches/asmFunctions.asm", "r") as file:
        asm = file.read()
    logdata = ""
...
def apply_asm(post_data):
    ...
    """
    loadASM = LuaRuntime(unpack_returned_tuples=False).eval("""function(code_filename)
