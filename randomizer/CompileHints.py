...
from randomizer.Enums.Settings import BarrierItems, HelmSetting, LogicType, MicrohintsEnabled, MoveRando, ShockwaveStatus, ShuffleLoadingZones, SpoilerHints, WinCondition, WrinklyHints
        ...
        while len(hintable_levels) == 0:
            ...
            if hint_location is not None:
                ...
                hintable_levels = [
                    level
                    for level in all_levels
                    if (not level_order_matters or spoiler.settings.BLockerEntryCount[level] > spoiler.settings.BLockerEntryCount[hint_location.level]) and (hint_location.level, level) not in hinted_blocker_combos
                ]
    ...
    if hint_distribution[HintType.TroffNScoff] > 0:
        ...
        while placed_tns_hints < hint_distribution[HintType.TroffNScoff]:
            ...
            while not any(future_tns_levels):
                ...
                hint_location = getRandomHintLocation()
                future_tns_levels = [
                    level for level in all_levels if level in levels_with_tns and (not level_order_matters or spoiler.settings.BLockerEntryCount[level] >= spoiler.settings.BLockerEntryCount[hint_location.level])
                ]
