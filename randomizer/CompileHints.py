...
def compileHints(spoiler: Spoiler) -> bool:
    ...
    # If we're doing the item-hinting system, use that distribution
        ...
        if (
            ...
            and (spoiler.settings.win_condition_item in (WinConditionComplex.beat_krool, WinConditionComplex.krools_challenge) or spoiler.settings.krool_ship_spawn_method == 1)
    ...
    # Otherwise we dynamically generate the hint distribution
        ...
        if spoiler.settings.shuffle_items and Types.Shop in spoiler.settings.shuffled_location_types:
            ...
            if spoiler.settings.logic_type not in (LogicType.nologic, LogicType.minimal):
                ...
                if spoiler.settings.win_condition_item in (WinConditionComplex.beat_krool, WinConditionComplex.krools_challenge) or spoiler.settings.krool_ship_spawn_method == 1:
