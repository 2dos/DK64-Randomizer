...
class FairyPlacementInfo:
    ...
    def __init__(self, location: Locations, level: Levels, internal_level_index: int, id: int, shift: int = -1, script_id: int = -1, map_id: int = -1):
...
def ShuffleFairyLocations(spoiler):
    ...
    if spoiler.settings.random_fairies:
        ...
        fairy_data_table = [
            ...
            FairyPlacementInfo(Locations.JapesBananaFairyLankyCave, Levels.JungleJapes, 1, 7, 0, 5, Maps.JapesLankyCave),
            ...
            FairyPlacementInfo(Locations.FactoryBananaFairybyFunky, Levels.FranticFactory, 0, 92, 1, 0x109, Maps.FranticFactory),
            FairyPlacementInfo(Locations.FactoryBananaFairybyCounting, Levels.FranticFactory, 1, 85),
            FairyPlacementInfo(Locations.GalleonBananaFairybyCranky, Levels.GloomyGalleon, 0, 21, 2, 0x45, Maps.GloomyGalleon),
            FairyPlacementInfo(Locations.GalleonBananaFairy5DoorShip, Levels.GloomyGalleon, 1, 8),
            FairyPlacementInfo(Locations.CavesBananaFairyIgloo, Levels.CrystalCaves, 0, 3, 5, 0x0, Maps.CavesTinyIgloo),
            FairyPlacementInfo(Locations.CavesBananaFairyCabin, Levels.CrystalCaves, 1, 3, 6, 0x5, Maps.CavesDiddyUpperCabin),
            FairyPlacementInfo(Locations.ForestBananaFairyRafters, Levels.FungiForest, 0, 2, 3, 0x0, Maps.ForestRafters),
            FairyPlacementInfo(Locations.ForestBananaFairyThornvines, Levels.FungiForest, 1, 2, 4, 0x24, Maps.ForestThornvineBarn),
            ...
            FairyPlacementInfo(Locations.IslesBananaFairyFactoryLobby, Levels.DKIsles, 0, 2, 7, 0xE, Maps.FranticFactoryLobby),
            FairyPlacementInfo(Locations.IslesBananaFairyForestLobby, Levels.DKIsles, 1, 1, 8, 0x5, Maps.FungiForestLobby),
                ...
                for index, data in enumerate(fairy_data_table):
                    if data.level == level and data.internal_level_index == slot:
                        ...
                        spoiler.fairy_data_table[index] = {
                            ...
                            "script_id": -1 if not is_vanilla else data.script_id,
                            "map_id": -1 if not is_vanilla else data.map_id,
                        }
                        # Insert into logic
