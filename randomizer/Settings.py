    ...
    def update_valid_locations(self):
        ...
        if self.shuffle_items and any(self.shuffled_location_types):
            ...
            if Types.JunkItem in self.shuffled_location_types:
                self.valid_locations[Types.JunkItem] = [x for x in fairyBannedLocations if LocationList[x].type not in (Types.Shop, Types.Crown) and (LocationList[x].type != Types.Key or LocationList[x].level == Levels.HideoutHelm)]
