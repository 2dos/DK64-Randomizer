...
QoLItems = [
    MultiselectorItem("Auto Dance Skip", 4, "Dances upon picking up some collectables, notably Golden Bananas, are removed (with some exceptions)."),
    ...
    MultiselectorItem("Animal Buddies grab Items", 13, "Rambi and Enguarde will be able to pick up DK's and Lanky's Items respectively."),
    ...
    MultiselectorItem("Hint Textbox Hold", 11, "Hint Textboxes will not close automatically upon the game reaching the end of the text, requiring B to be pressed."),
    MultiselectorItem("Remove Wrinkly Puzzles", -1, "Removes the Wrinkly Puzzles from the Angry Aztec, Fungi Forest and Crystal Caves lobbies"),
    MultiselectorItem("Fast Picture Taking", 2, "The picture taking sequence is heavily sped up, with lag being significantly reduced on BizHawk."),
    MultiselectorItem("HUD Hotkey", 14, "Pressing D-Pad Up will show the total amount of colored bananas acquired in the level, as well as the blueprint count for that Kong."),
    MultiselectorItem("Ammo Swap", 7, "Homing Ammo and Standard ammo can be swapped between (upon having Homing Ammo) by pressing D-Pad Down."),
    MultiselectorItem("Homing Balloons", 15, "Homing Ammo homes in on Banana Balloons."),
    MultiselectorItem("Fast Transform Animation", 6, "Transform barrels will not go through the morphing animation."),
    MultiselectorItem("Troff n Scoff Audio Indicator", 8, "A bell ding will play upon collecting enough colored bananas to unlock the level's boss."),
    MultiselectorItem("Lowered Aztec Lobby Bonus", 3, "The bonus barrel in Aztec Lobby is lowered to make it easier to reach."),
    MultiselectorItem("Quicker Galleon Star", 9, "The star in Gloomy Galleon now only requires Enguarde to go through it once to open the Gold Tower Gate."),
    MultiselectorItem("Vanilla Bug Fixes", 10, "Various bugs in the vanilla game have been fixed."),
    MultiselectorItem("Save K Rool Progress", 16, "Re-Entering K Rool after dying or pause exiting will spawn you in the latest phase you reached."),
    MultiselectorItem("Small Bananas always visible", 17, "Small Bananas will always be visible regardless of whether you have the kong unlocked or not."),
    MultiselectorItem("Fast Hints", 19, "Wrinkly will appear faster out of her door. Additionally, pressing A during any text bubble growth will skip to it's fully grown state."),
    MultiselectorItem("Brighten Mad Maze Maul Enemies", 20, "Enemies in Mad Maze Maul will be at full brightness, making them easier to see in dark areas."),
    MultiselectorItem("Raise Fungi Dirt Patch", -1, "The Fungi Dirt Patch near the mill that was discovered in 2017 is slightly raised to make it visible."),
...
FasterCheckItems = [
    ...
    MultiselectorItem("Factory Piano Game", 0, "The piano game in Factory will only require the 3, 5 and 7 long sequences to be recalled."),
    MultiselectorItem("Factory Diddy RnD", 1, "Only one enemy wave will need to be completed in the pincode room."),
    MultiselectorItem("Factory Arcade Round 1", 3, "The item for beating Round 1 will be stored inside the Baboon Blast course instead."),
    ...
    MultiselectorItem("Forest Rabbit Race", 4, "You will only be tasked with beating the 2nd rabbit race. Beating that race will give you both the reward for the 1st and 2nd races."),
    ...
    MultiselectorItem("Arcade", 6, "DK Arcade will only consist of 2 screens rather than 4, with 75m being an excluded screen from appearing in the first two."),
]
