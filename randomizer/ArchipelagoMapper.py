...
class ArchipelagoMapper:
    ...
    SKIP_SETTINGS = {
        'music_bgm_randomized', 'music_majoritems_randomized', 'music_minoritems_randomized',
        'music_events_randomized', 'random_music', 'music_rando_enabled', 'music_is_custom',
        'music_vanilla_locations', 'music_disable_reverb', 'isles_cool_musical',
        'music_selection_dict', 'music_selections', 'bgm_songs_selected',
        'majoritems_songs_selected', 'minoritems_songs_selected', 'events_songs_selected',
        'kong_model', 'random_models', 'color_palettes', 'randomize_pickups',
        'randomize_enemies', 'krool_model', 'misc_model_changes', 'texture_rando',
        'disable_tag_barrels', 'fps_display', 'enemy_rando', 'colorblind_mode',
        'disco_chunky', 'krusha_mode', 'crown_mode', 'puzzle_mode', 'kasplat_rando',
        'enemy_drop_list',
    }
        ...
        if 'Range' in parent_class or 'NamedRange' in parent_class:
            ...
            if range_start and range_end:
                option_info['metadata'] = {
                    'min': int(range_start.group(1)),
                    'max': int(range_end.group(1))
                }
            if default:
    ...
    def _parse_fillsettings_mappings(self) -> Dict[str, str]:
        ...
        fillsettings_path = os.path.join(
            os.path.dirname(__file__),
            '..',
            'archipelago',
            'FillSettings.py'
    ...
    def settings_to_yaml(self, settings_dict: Dict, player_name: str = "Player", 
                        game_version: str = "0.6.6") -> str:
        """
    ...
    def generate_triggers(self, settings_dict: Dict) -> List[Dict]:
        ...
        if starting_kongs_count > 1:
            ...
            triggers.append({
                'option_category': 'Donkey Kong 64',
                'option_name': 'starting_kong_count',
                'option_result': starting_kongs_count,
                'options': {
                    'Donkey Kong 64': {
...
def export_to_yaml(settings_dict: Dict, player_name: str = "Player", 
                   include_triggers: bool = True, game_version: str = "0.6.6") -> str:
    """
    ...
    if include_triggers:
        ...
        if triggers:
            ...
            yaml_str_with_triggers = yaml.dump(yaml_dict, default_flow_style=False, 
                                              sort_keys=False, allow_unicode=True, width=120)
            
            # Add header back
            header = (
                "# Donkey Kong 64 Randomizer Settings\n"
