...
from randomizer.Patching.Library.ASM import getItemTableWriteAddress, populateOverlayOffsets, getROMAddress, getSym, Overlay
...
def getItemPatchingData(item_type: Types, item: Items) -> ItemPatchingInfo:
    ...
    if item_type in simple_types:
    ...
    elif item_type in (Types.Shockwave, Types.Shop, Types.Climbing, Types.TrainingBarrel, Types.MelonUpgrade):
        # Special Moves
        ...
        # Melon Upgrades
        if item in (Items.MelonUpgrade, Items.MelonUpgrade1):
            return ItemPatchingInfo(2, 12, 0, 6)
        # Misc flag moves
...
def place_randomized_items(spoiler, original_flut: list, ROM_COPY: LocalROM):
    ...
    if spoiler.settings.shuffle_items:
        ...
        if spoiler.settings.item_reward_previews:
            ...
            for item in item_data:
                if item.location in direct_rom_write:
                    reward_text = getItemPreviewText(item.new_item, item.location, True, getModelMask(item.new_subitem))
                    addr = getROMAddress(getSym(direct_rom_write[item.location]), Overlay.Custom, offset_dict)
                    ROM_COPY.seek(addr)
                    ROM_COPY.writeBytes(bytes(reward_text, "ascii"))
