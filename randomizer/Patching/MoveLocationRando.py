    ...
    if spoiler.settings.microhints_enabled != MicrohintsEnabled.off:
        if kong != Kongs.any or slot == 0:
            ...
            hinted_items = {
                ...
                Items.ProgressiveSlam: [("slam", 1, Kongs.any), 33, [MicrohintsEnabled.all]],
            }
            for item_hint in hinted_items:
                move_data = hinted_items[item_hint][0]
                if (move_dict["move_type"] == move_data[0] and move_dict["move_lvl"] == move_data[1] and move_dict["move_kong"] == move_data[2]) or (move_data["move_type"] == move_data[0] and move_data[0] == "slam"):
