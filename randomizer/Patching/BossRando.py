...
def randomize_bosses(spoiler):
    ...
    ROM_COPY.seek(varspaceOffset + bossMapOffset)
    ROM_COPY.writeBytes(bytearray(spoiler.settings.boss_kongs))
    ROM_COPY.writeBytes(bytearray(spoiler.settings.boss_maps))
