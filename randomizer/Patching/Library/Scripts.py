...
def getHelmLobbyActivatorScript(item_id: int, activator: SwitchsanityGone, bonus_map: int, microhint: bool) -> list[int]:
    ...
    return compileInstanceScript(
        ...
        {"activator": activator, "is_instrument": activator >= SwitchsanityGone.bongos and activator <= SwitchsanityGone.triangle, "microhint": microhint},
    )
    ...
    return compileInstanceScript(
        ...
        [
            ...
            ScriptBlock(
                ...
                + [IScript_SetExternalScriptRunState(x, RunState.pause, 0, lambda d: d["fast"]) for x in remaining_door_ids]  # Disable other doors
                + [
...
def addNewScript(ROM_COPY: LocalROM, cont_map_id: int, item_ids: list[int], stype: ScriptTypes, extra_data: dict = {}) -> None:
    ...
    for item_id in item_ids:
        ...
        if stype == ScriptTypes.Bananaport:
            ...
            subscript = getHelmPadScript(
                item_id,
                extra_data[item_id]["temp_flags"],
                extra_data[item_id]["kong_id"],
                extra_data[item_id]["glass_panel"],
                extra_data[item_id]["hint_cs"],
                extra_data[item_id]["microhint"],
                extra_data[item_id]["req_minigames"],
                extra_data[item_id]["helm_order"],
            )
