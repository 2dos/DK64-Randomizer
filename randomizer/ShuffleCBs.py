        ...
        for balloon in balloon_lst:
            if placed_balloons < selected_balloon_count:
                ...
                if len(balloon_kongs) > 0:  # Has a kong who can be assigned to this balloon
                    ...
                    level_placement.append({"name": balloon.name, "kong": selected_kong, "level": level, "type": "balloons", "map": balloon.map})
                    placed_balloons += 1
        # Model Two CBs
        ...
        for item in modeltwo_cb_lst:
            ...
            if group_weight % 5 == 0:  # TODO: Remove this, this is only to make sure 100-CB groups generate
                ...
                if len(cb_kongs) > 0 and selected_single_count >= placed_singles + singles_in_group:
                    ...
                    level_placement.append({"name": item.name, "kong": selected_kong, "level": level, "type": "cb", "map": item.map})
