...
with open("dirt_patches.csv", newline="") as csvfile:
    ...
    for idx, row in enumerate(patch_data):
        if idx > 0:
            patch_data_json.append({"map": row[1], "levelname": row[2], "subname": row[3], "name": row[4], "coords": [float(row[5]), float(row[6]), float(row[7])], "rot": int(row[9]), "vanilla": row[10] == "YES", "group": int(row[11]), "logicregion": row[12], "logic": row[13], "resize": row[15]})
