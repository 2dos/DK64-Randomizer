...
def createHTML(markdown_file_name: str, markdown_path: str, template_text: str):
    ...
    html_text = template_text.replace("<title></title>", f"<title>{convertName(entry_name)}</title>")
    html_text = template_text.replace('<h1 id="page-title"></h1>', f'<h1 id="page-title">{convertName(entry_name)}</h1>')
