...
def validateSeed(level_order,all_kongs,all_moves,fast_start,b_locker_array,tns_array,castle_kongs):
	...
	moves_level_purchasing = [
		# Japes
		["dk_cranky","diddy_cranky","lanky_cranky","tiny_cranky","chunky_cranky","gun_dk","gun_diddy","gun_lanky","gun_tiny","gun_chunky"],
		# Aztec
	...
	while loop_control:
		...
		for i in range(len(banana_cache["isles"])):
			...
			if not x["collected"]:
				passes = checkMoves("isles",x.copy(),moves_acquired.copy(),kongs_unlocked.copy(),lobby_access.copy(),keys_acquired.copy())
				if passes:
					gbs_collected += 1
					x["collected"] = True
					# print(x["name"])
				...
				if level_order[i] in lobby_access:
					if gbs_collected > b_locker_array[level_order[i]]:
						...
						for j in range(len(banana_cache[level_name])):
							...
							if not x["collected"]:
								passes = checkMoves(level_name,x.copy(),moves_acquired.copy(),kongs_unlocked.copy(),lobby_access.copy(),keys_acquired.copy())
								if passes:
									second_pass = False;
									if x["frees_dk"]:
										second_pass = True;
